...
namespace activemq_logic {	
	...
	void CrawlerInfoConsumer::run() {
		try {
			...
			if (use_topic) {
                ...
                destination = session->createQueue("CrawlerInfoSet1201?consumer.prefetchSize=1");
            }
	...
	void SendSingleInfo(CrawlerInfo* sent_crawler_info) {
		activemq::library::ActiveMQCPP::initializeLibrary();
		{
			...
			"failover:(tcp://192.168.0.2:61616"
            //"?wireFormat=openwire"
			...
			CrawlerInfoProducer producer(broker_URI, numMessages, use_topic, session_transacted);
	...
	CrawlerInfo RecvSingleInfo() {
		...
		activemq::library::ActiveMQCPP::initializeLibrary();
		{
			...
			CrawlerInfoConsumer consumer(broker_URI, numMessages, use_topic, session_transacted);
