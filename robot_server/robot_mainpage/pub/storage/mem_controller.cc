...
MemController::MemController() {
    engine_ =
        base_storage::DictionaryStorageEngine::Create(base_storage::IMPL_MEM);
}
...
bool MemController::WriteKeyValueData(base_logic::Value* value) {
    ...
    r = engine_->SetValue(key.c_str(), key.length(),
            binary->GetBuffer(), binary->GetSize());
    return r;
}
...
bool MemController::ReadBatchKeyValue(base_logic::Value* value) {
    ...
    while (engine_->FetchValue(return_key, &return_key_length, &return_value,
        ...
        if (return_key && return_value) {
            list->Append(base_logic::Value::CreateBinaryValue(return_value,
                    return_value_length));
        }
    }
    if (return_value)
