...
void CrawlerInfoConsumer::onMessage(const Message* message) {
  ...
  try {
    ...
    if (textMessage != NULL) {
      ...
      crawlerInfoReceiver->curDepth = textMessage->getIntProperty("curDepth");
      crawlerInfoReceiver->URL = textMessage->getStringProperty("URL");
      crawlerInfoReceiver->maxDepth = textMessage->getIntProperty("maxDepth");
...
void SendSingleInfo(CrawlerInfo* infoNeedSent) {
  ...
  {
    std::string brokerURI = "failover:(tcp://112.124.49.59:61616"
//        "?wireFormat=openwire"
...
//        "&wireFormat.tightEncodingEnabled=true"
    ...
    CrawlerInfoConsumer consumer(brokerURI, numMessages, useTopics,
                                 sessionTransacted);
