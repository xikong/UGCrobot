...
class ForgeryUA {
 public:
    ...
    const int64 count() const {return data_->count_;}
    int64 send_time() const {return data_->send_time_;}
    void ValueSerialization(base_logic::DictionaryValue* dict);
...
//	};
	...
	static const int64 WEIBO = 7006;
	static const int64 TIANYA = 7005;
	static const int64 TIEBA = 7001;
	static const int64 QZONE = 7003;
	static const int64 MAIN = 30001;

	RobotTask(): id_(0), type_(0), state_(TASK_WAIT), send_time_(0), create_time_(time(NULL)) {}
	...
	void set_type(int64 type) {type_ = type;}
	void set_state(int state) {state_ = state;}
	int state() const {return state_;}
	void set_send_time(int32 send_time) {send_time_ = send_time;}
	int32 send_time() {return send_time_;};
	void set_row_key(std::string row_key) {row_key_ = row_key;}
	std::string row_key() const {return row_key_;}
	void set_create_time(time_t t = time(NULL)) {create_time_ = time(NULL);}
...
public:
	TiebaTask(): polling_time_(10),
				 base_polling_time_(10),
				 last_task_time_(time(NULL)) {type_ = TIEBA;}
...
public:
	...
	void set_ua(const std::string &ua) {ua_ = ua;}
    ...
    void update_time(const int64 update_time = 0,
        ...
        if (is_first)
            polling_time_ =
                    (time(NULL) + radom_num) % base_polling_time_;
        else
            polling_time_ =
                (time(NULL) + radom_num) % base_polling_time_
                + base_polling_time_;
