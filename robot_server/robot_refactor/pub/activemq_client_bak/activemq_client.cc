...
void CrawlerInfoProducer::run() {
	try {
		...
		// 创建线程ID（测试用）
		std::string threadIdStr = Long::toString(
				Thread::currentThread()->getId());

		// 创建信息
		std::string text = (std::string) "Test Message! from thread "
				+ threadIdStr;
		...
		for (int ix = 0; ix < MessageNum; ++ix) {
			...
			message->setByteProperty("max_depth",
					this->crawler_info->max_depth);
			message->setByteProperty("cur_depth",
					this->crawler_info->cur_depth);
...
void CrawlerInfoConsumer::onMessage(const Message* message) {
	...
	try {
		...
		if (text_message != NULL) {
			...
			recv_crawler_info->task_id = text_message->getLongProperty(
					"task_id");
			recv_crawler_info->attr_id = text_message->getIntProperty(
					"attr_id");
			recv_crawler_info->max_depth = text_message->getByteProperty(
					"max_depth");
			recv_crawler_info->cur_depth = text_message->getByteProperty(
					"cur_depth");
