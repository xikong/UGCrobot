...
bool PacketProsess::PacketStream(const PacketHead* packet_head,
	...
	case PLUGIN_SVC_MGR_REG: {
		struct PluginSvcMgrReg *plg_svc_reg =
				(struct PluginSvcMgrReg *) packet_head;
	...
	case ASSIGN_WEIBO_TASK: {
		struct AssingWeiboTask *weibo_task =
				(struct AssingWeiboTask*) packet_head;
		...
		BUILDHEAD(data_len);
		out.Write16(weibo_task->task_type);
	...
	case ASSIGN_TIANYA_TASK: {
		...
		std::list<struct TianyaTaskUnit*>::iterator it =
				tianya_task->task_set.begin();
		...
		BUILDHEAD(data_len);
		out.Write16(tianya_task->task_type);
		...
		for (; tianya_task->task_set.end() != it; ++it) {
			...
			out.WriteData((*it)->pre_user_name.c_str(), (*it)->pre_user_name.size());
		}
		body_stream = const_cast<char*>(out.GetData());
		break;
	}
	case ASSIGN_TIEBA_TASK: {
		...
		struct AssignTiebaTask *tieba_task =
				(struct AssignTiebaTask*) packet_head;
		...
		for (; tieba_task->task_set.end() != it; ++it) {
			...
			data_len += (*it)->pre_url.size();
			data_len += (*it)->kw.size();
			data_len += (*it)->fid.size();
			data_len += (*it)->tbs.size();
			data_len += (*it)->repost_id.size();
		}
		BUILDHEAD(data_len);
		out.Write16(tieba_task->task_type);
		...
		for (; tieba_task->task_set.end() != it; ++it) {
			...
			out.Write16((*it)->kw.size());
			out.WriteData((*it)->kw.c_str(), (*it)->kw.size());

			out.Write16((*it)->fid.size());
			out.WriteData((*it)->fid.c_str(), (*it)->fid.size());

			out.Write16((*it)->tbs.size());
			out.WriteData((*it)->tbs.c_str(), (*it)->tbs.size());

			out.Write32((*it)->floor_num);

			out.Write16((*it)->repost_id.size());
			out.WriteData((*it)->repost_id.c_str(), (*it)->repost_id.size());
		}
		body_stream = const_cast<char*>(out.GetData());
		break;
	}
	case ASSIGN_QZONE_TASK: {
		struct AssignQzoneTask *qzone_task =
				(struct AssignQzoneTask*) packet_head;
		...
		BUILDHEAD(data_len);
		out.Write16(qzone_task->task_type);
	...
	case ASSIGN_ROBOT_TASKS: {
		struct RobotTasks *tasks =
				(struct RobotTasks*) packet_head;
		uint16 data_len = ROBOT_TASKS_SIZE;
		std::list<struct RobotTaskBase*>::iterator it =
				tasks->task_set.begin();
		for (; tasks->task_set.end() != it; ++it) {
			data_len += (*it)->size();
		}
		BUILDHEAD(data_len);
		out.Write16(tasks->task_num);
		it = tasks->task_set.begin();
		for (; tasks->task_set.end() != it; ++it) {
			(*it)->SerializeSelf(out);
		}
		body_stream = const_cast<char*>(out.GetData());
		break;
	...
	if (r) {
		...
		if (is_zip_encrypt == ZIP_AND_NOENCRYPT) {
			...
			zip_len = CompressionStream(unzip,
					packet_length - sizeof(int16) - sizeof(int8), &zip_data);
			free(body_stream);
			body_stream = NULL;
...
bool PacketProsess::UnpackStream(const void* packet_stream, int32 len,
	...
	case ANALYTICAL_REG: {
		...
		*packet_head = (struct PacketHead*) vCrawlerMgrReg;
		...
		MAC_SIZE - 1);
		int32 mac_len =
				(temp - 1) < (MAC_SIZE - 1) ? (temp - 1) : (MAC_SIZE - 1);
...
//			LOG_DEBUG2("new RouterStatusUnit: %X", unit);
	...
	case ROBOT_TASK_STATE: {
		...
		*packet_head = status;
		...
		status->error_no_len = in.Read16();
		int remain_bytes = 0;
		status->error_no = std::string(
				in.ReadData(status->error_no_len, remain_bytes),
				status->error_no_len);
	...
	case ROUTER_SCHEDULE_FAIL: {
		struct RouterScheduleFailStatus *status =
				new RouterScheduleFailStatus();
		*packet_head = status;
		FILLHEAD();
		status->router_id = in.Read32();
		status->status = in.Read8();
	}
	default:
		r = false;
		break;
...
//		LOG_DEBUG2("delete RouterStatus: %X", router_status);
	...
	case ROBOT_TASK_STATE: {
		struct RobotTaskStatus *status =
				(struct RobotTaskStatus*)packet_head;
		delete status;
		break;
	}
	case ASSIGN_TIANYA_TASK: {
	...
	case ROUTER_SCHEDULE_FAIL: {
		struct RouterScheduleFailStatus *status =
				(struct RouterScheduleFailStatus*)packet_head;
		delete status;
		break;
	}
	default:
...
void PacketProsess::DumpPacket(const struct PacketHead* packet_head) {
	...
	case PLUGIN_SVC_MGR_REG: {
		struct PluginSvcMgrReg *svc_reg = (struct PluginSvcMgrReg *) packet_head;
	...
	case ASSIGN_WEIBO_TASK: {
		...
		struct AssingWeiboTask *weibo_task =
				(struct AssingWeiboTask*) packet_head;
		PRINT_INT(weibo_task->task_type);
	...
	case ASSIGN_TIANYA_TASK: {
		...
		struct AssignTianyaTask *tianya_task =
				(struct AssignTianyaTask*) packet_head;
		PRINT_INT(tianya_task->task_type);
	...
	case ASSIGN_TIEBA_TASK: {
		...
		struct AssignTiebaTask *tieba_task =
				(struct AssignTiebaTask*) packet_head;
		PRINT_INT(tieba_task->task_type);
		...
		for (; tieba_task->task_set.end() != it; ++it) {
			...
			PRINT_INT((*it)->kw.size());
			PRINT_STRING((*it)->kw.c_str());

			PRINT_INT((*it)->fid.size());
			PRINT_STRING((*it)->fid.c_str());

			PRINT_INT((*it)->tbs.size());
			PRINT_STRING((*it)->tbs.c_str());

			PRINT_INT((*it)->floor_num);

			PRINT_INT((*it)->repost_id.size());
			PRINT_STRING((*it)->repost_id.c_str());
		}
	...
	case ASSIGN_QZONE_TASK: {
		...
		struct AssignQzoneTask *qzone_task =
				(struct AssignQzoneTask*) packet_head;
		PRINT_INT(qzone_task->task_type);
	...
	case ASSIGN_ROBOT_TASKS: {
		PRINT_TITLE("struct RobotTasks Dump Begin");
		DUMPHEAD();
		struct RobotTasks *tasks =
				(struct RobotTasks*) packet_head;
		PRINT_INT(tasks->task_num);
		std::list<struct RobotTaskBase*>::iterator it =
				tasks->task_set.begin();
		for (; tasks->task_set.end() != it; ++it) {
			j += (*it)->Print(buf+j, DUMPPACKBUF-j);
		}
		PRINT_END("struct RobotTasks Dump End");
		break;
	}
	case ROBOT_TASK_STATE: {
		PRINT_TITLE("struct RobotTaskState Dump Begin");
		DUMPHEAD();
		struct RobotTaskStatus *states =
				(struct RobotTaskStatus *)packet_head;
		PRINT_INT(states->task_type);
		PRINT_INT(states->is_success);
		PRINT_INT(states->error_no_len);
		PRINT_STRING(states->error_no.c_str());
		PRINT_INT64(states->task_id);
		PRINT_INT64(states->cookie_id);
		PRINT_END("struct RobotTaskState Dump End");
		break;
	}
	default:
		break;
...
void PacketProsess::ClearDetailTaskList(struct PacketHead* packet_head) {
	struct AssignmentDetailTask* task =
			(struct AssignmentDetailTask*) packet_head;
	while (task->task_set.size() > 0) {
		struct TaskUnit* unit = task->task_set.front();
		task->task_set.pop_front();
...
void PacketProsess::ClearRobotTaskList(struct PacketHead* packet_head) {
//	using base_logic::RobotTask;
	struct RobotTasks *tasks = (struct RobotTasks*) packet_head;
	while (tasks->task_set.size() > 0) {
		struct RobotTaskBase *base = tasks->task_set.front();
		tasks->task_set.pop_front();
		base_logic::RobotTask::TaskType type = (base_logic::RobotTask::TaskType)base->task_type;
		switch (type) {
		case base_logic::RobotTask::TIEBA: {
			struct TiebaTask *tieba = (struct TiebaTask *) base;
			delete tieba;
			break;
		}
		case base_logic::RobotTask::TAOGUBA: {
			struct TaogubaTask *taoguba = (struct TaogubaTask *) base;
			delete taoguba;
			break;
		}
