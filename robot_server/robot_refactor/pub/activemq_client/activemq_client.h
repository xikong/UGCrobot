...
public:
	...
	CrawlerInfoProducer(const std::string& broker_URI, int MessageNum,
			...
			connection(NULL), session(NULL), destination(NULL), producer(NULL), MessageNum(
					MessageNum), use_topic(use_topic), session_transacted(
					session_transacted), broker_URI(broker_URI) {
...
public:
	...
	CrawlerInfoConsumer(const std::string& broker_URI, int numMessages,
			bool use_topic = false, bool session_transacted = false,
			int waitMillis = 2000) :
			latch(1), doneLatch(numMessages), connection(NULL), session(NULL), destination(
					NULL), consumer(NULL), waitMillis(waitMillis), use_topic(
					use_topic), session_transacted(session_transacted), broker_URI(
					broker_URI) {
	}
