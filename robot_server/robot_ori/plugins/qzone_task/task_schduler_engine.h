...
typedef std::map<int64, base_logic::QZoneTask> TASKINFO_MAP;
typedef std::list<base_logic::QZoneTask>  TASKINFO_LIST;
...
class TaskSchdulerManager {
 public:
    ...
    void InitDB(qzone_task_logic::CrawlerTaskDB*     task_db);

    void InitManagerInfo(plugin_share::ManagerInfo *info);

    void FetchBatchTask(std::list<base_logic::QZoneTask> *list,
            bool is_first = false);
 ...
 private:
    ...
    qzone_task_logic::CrawlerTaskDB*     task_db_;
