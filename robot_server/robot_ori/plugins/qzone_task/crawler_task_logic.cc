...
#define DEFAULT_CONFIG_PATH     "./plugins/qzone_task/qzone_task_config.xml"
...
base_logic::RobotTask::TaskType CrawlerTasklogic::task_type_ = base_logic::RobotTask::QZONE;
...
bool CrawlerTasklogic::Init() {
	...
	task_db_.reset(new qzone_task_logic::CrawlerTaskDB());
	task_time_mgr_.reset(
			new qzone_task_logic::TaskTimeManager(task_db_.get()));
...
router_schduler_engine_	= (*router_engine)();
	...
	qzone_task_logic::TaskSchdulerManager* schduler_mgr =
			qzone_task_logic::TaskSchdulerEngine::GetTaskSchdulerManager();
	...
	qzone_task_logic::TaskSchdulerEngine* engine =
			qzone_task_logic::TaskSchdulerEngine::GetTaskSchdulerEngine();
...
bool CrawlerTasklogic::OnIniTimer(struct server *srv) {
	if (srv->add_time_task != NULL) {
		srv->add_time_task(srv, "qzone_task", TIMER_SERVER_STARTUP, 1, 1);
		srv->add_time_task(srv, "qzone_task", TIME_DISTRIBUTION_TASK, 60, -1);
		srv->add_time_task(srv, "qzone_task", TIME_FECTCH_TASK, 10, -1);
		srv->add_time_task(srv, "qzone_task", TIME_RECYCLINGTASK, 20, -1);
		srv->add_time_task(srv, "qzone_task", TIME_FETCH_TEMP_TASK, 8, -1);
		srv->add_time_task(srv, "qzone_task", TIME_DISTRBUTION_TEMP_TASK, 10,
				-1);
		srv->add_time_task(srv, "qzone_task", TIME_CLEAN_NO_EFFECTIVE, 20,
				-1);
		srv->add_time_task(srv, "qzone_task", TIME_UPDATE_EXEC_TASKS, 10, -1);
	}
	return true;
}
...
bool CrawlerTasklogic::OnRobotTaskStatus(struct server* srv, int socket,
	...
	if (task_type_ != base_logic::RobotTask::TaskType(status->task_type))
		return false;
	task_db_->UpdateRobotTaskState(status->task_id, status->is_success);
