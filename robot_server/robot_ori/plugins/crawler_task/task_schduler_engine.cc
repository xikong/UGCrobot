...
//    cookie_cache_->SortCookies();
    ...
    //======================== Test ========================
    while ((*list).size() > 0) {
        base_logic::WeiboTaskInfo info = (*list).front();
        (*list).pop_front();
        // 更新时间
...
bool TaskSchdulerManager::DistributionTask() {
    ...
    for (; it != task_cache_->task_idle_map_.end(), index < count; index++) {
    	base_logic::LoginCookie cookie;
        base_logic::WeiboTaskInfo& info = it->second;
        if (cookie_cache_->GetCookie(info.attrid(), cookie)) {
			task_db_->RecordWeiboTaskState(info);
