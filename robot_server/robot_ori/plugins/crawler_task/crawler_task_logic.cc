...
bool CrawlerTasklogic::OnIniTimer(struct server *srv) {
	if (srv->add_time_task != NULL) {
		srv->add_time_task(srv, "crawler_task", TIMER_SERVER_STARTUP, 1, 1);
		srv->add_time_task(srv, "crawler_task", TIME_DISTRIBUTION_TASK, 30, -1);
		...
		srv->add_time_task(srv, "crawler_task", TIME_DISTRBUTION_TEMP_TASK, 10,
				-1);
		srv->add_time_task(srv, "crawler_task", TIME_CLEAN_NO_EFFECTIVE, 20,
				-1);
		srv->add_time_task(srv, "crawler_task", TIME_UPDATE_EXEC_TASKS, 10, -1);
	}
	return true;
}
...
bool CrawlerTasklogic::OnRobotTaskStatus(struct server* srv, int socket,
	...
	LOG_DEBUG2("receive robot task status: task_id = %d, status = %d",
			status->task_id, status->is_success);
	task_db_->RecordRobotTaskResult(status->task_id, status->is_success);
}
}  // namespace crawler_task_logic
