...
bool CrawlerTaskKafka::FectchBatchTempTask(std::list<base_logic::WeiboTaskInfo>* list) {
    ...
    for (std::set<std::string>::iterator it = data_list.begin();
        ...
        base_logic::WeiboTaskInfo task_info;
...
void CrawlerTaskKafka::SetTaskInfo(base_logic::WeiboTaskInfo&task_info, base_logic::DictionaryValue* task_info_dic) {
	int64 temp_int;
	std::string temp_str;
