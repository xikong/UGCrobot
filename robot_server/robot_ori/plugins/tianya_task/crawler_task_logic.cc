...
#define DEFAULT_CONFIG_PATH     "./plugins/tianya_task/tianya_task_config.xml"
...
base_logic::RobotTask::TaskType CrawlerTasklogic::task_type_ = base_logic::RobotTask::TIANYA;
...
bool CrawlerTasklogic::Init() {
	...
	task_db_.reset(new tianya_task_logic::CrawlerTaskDB());
	task_time_mgr_.reset(
			new tianya_task_logic::TaskTimeManager(task_db_.get()));
...
router_schduler_engine_	= (*router_engine)();
	...
	tianya_task_logic::TaskSchdulerManager* schduler_mgr =
			tianya_task_logic::TaskSchdulerEngine::GetTaskSchdulerManager();
	...
	tianya_task_logic::TaskSchdulerEngine* engine =
			tianya_task_logic::TaskSchdulerEngine::GetTaskSchdulerEngine();
...
bool CrawlerTasklogic::OnIniTimer(struct server *srv) {
	if (srv->add_time_task != NULL) {
		srv->add_time_task(srv, "tianya_task", TIMER_SERVER_STARTUP, 1, 1);
		srv->add_time_task(srv, "tianya_task", TIME_DISTRIBUTION_TASK, 60, -1);
		srv->add_time_task(srv, "tianya_task", TIME_FECTCH_TASK, 10, -1);
		srv->add_time_task(srv, "tianya_task", TIME_RECYCLINGTASK, 20, -1);
		srv->add_time_task(srv, "tianya_task", TIME_FETCH_TEMP_TASK, 8, -1);
		srv->add_time_task(srv, "tianya_task", TIME_DISTRBUTION_TEMP_TASK, 10,
				-1);
		srv->add_time_task(srv, "tianya_task", TIME_CLEAN_NO_EFFECTIVE, 20,
				-1);
		srv->add_time_task(srv, "tianya_task", TIME_UPDATE_EXEC_TASKS, 10, -1);
	}
	return true;
}
