...
typedef std::map<int64, base_logic::TianyaTask> TASKINFO_MAP;
typedef std::list<base_logic::TianyaTask>  TASKINFO_LIST;
...
class TaskSchdulerManager {
 public:
    ...
    void InitDB(tianya_task_logic::CrawlerTaskDB*     task_db);

    void InitManagerInfo(plugin_share::ManagerInfo *info);

    void FetchBatchTask(std::list<base_logic::TianyaTask> *list,
            bool is_first = false);

    void FetchBatchTemp(std::list<base_logic::TianyaTask>* list);
 ...
 private:
    ...
    tianya_task_logic::CrawlerTaskDB*     task_db_;
