...
static void *OnCrawlerTaskStart() {
    ...
    task->name = "weibo_task";

    task->version = "1.0.0";

    task->provider = "kerry";

    if (!weibo_task_logic::CrawlerTasklogic::GetInstance())
        assert(0);
...
static handler_t OnCrawlerTaskShutdown(struct server* srv, void* pd) {
    weibo_task_logic::CrawlerTasklogic::FreeInstance();
    return HANDLER_GO_ON;
}

static handler_t OnCrawlerTaskConnect(struct server *srv, int fd,
        void *data, int len) {
    weibo_task_logic::CrawlerTasklogic::GetInstance()->OnTaskConnect(srv, fd);
    return HANDLER_GO_ON;
}

static handler_t OnCrawlerTaskMessage(struct server *srv, int fd, void *data,
        int len) {
    weibo_task_logic::CrawlerTasklogic::GetInstance()->OnTaskMessage(srv, fd, data, len);
    return HANDLER_GO_ON;
}

static handler_t OnCrawlerTaskClose(struct server *srv, int fd) {
    weibo_task_logic::CrawlerTasklogic::GetInstance()->OnTaskClose(srv, fd);
    return HANDLER_GO_ON;
}
...
static handler_t OnBroadcastConnect(struct server* srv,
    ...
    weibo_task_logic::CrawlerTasklogic::GetInstance()->OnBroadcastConnect(srv,
            fd, data, len);
    return HANDLER_GO_ON;
}

static handler_t OnBroadcastClose(struct server* srv, int fd) {
    weibo_task_logic::CrawlerTasklogic::GetInstance()->OnBroadcastClose(srv, fd);
    return HANDLER_GO_ON;
}

static handler_t OnBroadcastMessage(struct server* srv, int fd,
        void *data, int len) {
    weibo_task_logic::CrawlerTasklogic::GetInstance()->OnBroadcastMessage(srv,
            fd, data, len);
    return HANDLER_GO_ON;
}

static handler_t OnIniTimer(struct server* srv) {
    weibo_task_logic::CrawlerTasklogic::GetInstance()->OnIniTimer(srv);
    return HANDLER_GO_ON;
}

static handler_t OnTimeOut(struct server* srv, char* id,
        int opcode, int time) {
    weibo_task_logic::CrawlerTasklogic::GetInstance()->OnTimeout(srv, id, opcode, time);
    return HANDLER_GO_ON;
}

int weibo_task_plugin_init(struct plugin *pl) {
