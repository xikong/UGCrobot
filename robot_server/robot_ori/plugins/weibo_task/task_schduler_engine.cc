...
#include "../weibo_task/task_schduler_engine.h"
...
void TaskSchdulerManager::InitDB(weibo_task_logic::CrawlerTaskDB* task_db) {
    task_db_ = task_db;
...
void TaskSchdulerManager::SetBatchContents() {
	...
	base_logic::WLockGd lk(lock_);
	task_db_->FetchBatchTaskContent((int16)base_logic::RobotTask::WEIBO, &list);
	while (list.size() > 0) {
...
void TaskSchdulerManager::FetchBatchTask(
        std::list<base_logic::WeiboTask>* list,
        bool is_first) {
    base_logic::WLockGd lk(lock_);
...
//    cookie_cache_->SortCookies();
    ...
    //======================== Test ========================
    base_logic::WeiboTask info;
    std::string str = "真不错，哈哈";
    info.set_content(str);
    str = "3498216287052366";
    info.set_topic_id(str);
    str = "2931197424";
    info.set_host_uin(str);
    list->push_back(info);
    //======================== Test ========================
    while ((*list).size() > 0) {
        base_logic::WeiboTask info = (*list).front();
        (*list).pop_front();
        // 更新时间
...
//        if (!cookie_cache_->GetCookie(info.attrid(), cookie))
//        	return ;
//        info.set_cookie_id(cookie.cookie_id());
//        info.set_cookie(cookie.get_cookie_body());
...
void TaskSchdulerManager::RecyclingTask() {
    ...
    for (; it != task_cache_->task_exec_map_.end();) {
        base_logic::WeiboTask &task = it->second;
...
bool TaskSchdulerManager::DistributionTask() {
    ...
    struct AssingWeiboTask task;
    MAKE_HEAD(task,ASSIGN_WEIBO_TASK, 0, 0, 0, 0, 0, 0, manager_info_->svr_info.id, 0);
    task.task_type = (uint16)base_logic::RobotTask::WEIBO;

    int crawler_type = task_db_->GetCrawlerTypeByOpCode(ASSIGN_WEIBO_TASK);
...
//            info.update_time(current_time, base::SysRadom::GetInstance()->GetRandomID());
            ...
            if (task.task_set.size() % base_num == 0 &&
				...
				data_length = 0;
				crawler_schduler_engine_->SendOptimalRouter((const void*)&task, 0, crawler_type);
				net::PacketProsess::ClearWeiboTaskList(&task);
			}
        } else {
        	++it;
        }
