...
bool SchdulerEngineImpl::CheckOptimalRouter() {
    RouterSchdulerManager* schduler_mgr =
            RouterSchdulerEngine::GetRouterSchdulerManager();
    return schduler_mgr->CheckOptimalRouter();
}
...
bool RouterSchdulerManager::CheckOptimalRouter() {
    base_logic::RLockGd lk(lock_);
    return schduler_cache_->router_scheduler_id_map_.size() > 0?true:false;
}
