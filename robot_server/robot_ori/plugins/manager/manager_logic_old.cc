...
bool Managerlogic::HandleAllConnect(struct server *srv, const int socket) {
	time_t t = time(NULL);
	std::string addr;
...
bool Managerlogic::HandleAllMessage(struct server *srv, const int socket,
...
bool Managerlogic::HandleAllClose(struct server *srv, const int socket) {
	time_t t = time(NULL);
	std::string addr;
	logic::SomeUtils::GetAddressBySocket(socket, addr);
...
bool Managerlogic::OnSelfRegState(struct server* srv, int socket,
	...
	if ('\0' == reg_state->token[0]) {
		...
		ret = true;
		LOG_MSG("plugin server register success");
