...
class CrawlerInfoProducer: public Runnable {
 public:
	...
	CrawlerInfoProducer(const std::string& broker_URI, int MessageNum, bool use_topic = false, bool session_transacted = false) :
        connection(NULL),
        session(NULL),
        destination(NULL),
        producer(NULL),
...
class CrawlerInfoConsumer : public ExceptionListener,
 ...
 public:
	...
	CrawlerInfoConsumer(const std::string& broker_URI, int numMessages, bool use_topic = false, bool session_transacted = false, int waitMillis =2000) :
        latch(1),
