...
std::string RobotTask::GetContentFromDb(base_logic::DictionaryValue* dict) {
	dict->GetString(L"content", &content_);
}
...
void TiebaTask::GetDataFromKafka(base_logic::DictionaryValue *dict) {
	...
	floor_num_ = tmp;
//	dict->GetInteger(L"floor_num", &floor_num_);
	dict->GetString(L"content", &content_);
...
std::string TiebaTask::SerializeSelf() {
	std::stringstream os;
	os << RobotTask::SerializeSelf();
	os << ", url: " << url_ << ", kw: " << kw_ << ", fid: " << fid_ <<
			", tbs: " << tbs_ << ", floor_num: " << floor_num_ <<
			", content: " << content_ << ", repost_id: " << repost_id_ <<
			", addr: " << addr_;
	return os.str();
}
}  // namespace base_logic
