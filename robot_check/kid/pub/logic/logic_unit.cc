...
void CoreSoUtils::InitSRV() {
    ...
    CoreGetServer entry_point = (CoreGetServer)
        basic::get_function_pointer(handle_lancher,"get_server");
    if (entry_point==NULL){
...
int SomeUtils::GetAddressBySocket(const int sock, std::string &ip){
    ...
    int salen = sizeof(sa);
    if (::getpeername(sock, (struct sockaddr*)&sa, (socklen_t *)&salen) == -1) {
        ip = "?";
        return -1;
    }
...
int SomeUtils::GetPortBySocket(const int socket, int16 &port){
    struct sockaddr_storage sa;
    int salen = sizeof(sa);
    if (::getpeername(socket, (struct sockaddr*)&sa, (socklen_t *)&salen) == -1) {
        port = -1;
        return -1;
    }
...
bool SomeUtils::FindStrFromString(string &find, const string &src,
