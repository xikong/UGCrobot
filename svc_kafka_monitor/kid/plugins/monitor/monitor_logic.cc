...
const string MonitorLogic::g_default_config_path_ = "./plugins/monitor/monitor_config.xml";
...
bool MonitorLogic::OnIniTimer(struct server *srv){
    if(NULL != srv->add_time_task){
        ...
        srv->add_time_task(srv, "monitor", FETCH_NEW_KAFKA_TOPIC_INTERVAL, 15, -1);
        srv->add_time_task(srv, "monitor", RECORD_KAFKA_TOPIC_INTERVAL, 10, -1);
    }
}

bool MonitorLogic::OnTimeout(struct server *srv, char* id, int opcode, int time){
    switch(opcode){
