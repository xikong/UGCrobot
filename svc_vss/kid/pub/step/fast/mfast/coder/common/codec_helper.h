...
#include <stdexcept>
#include "../../../../fast/mfast/exceptions.h"
#include "../../../../fast/mfast/string_ref.h"
...
namespace mfast
{
  namespace detail
  {
    ...
    public:
      ...
      template <typename T>
      void save_previous_value(const T& mref) const
      {
        mref.save_to(previous_value_of(mref));
      }

      template <typename T>
      void load_previous_value(const T& mref) const
      {
      ...
      void
      copy_string_raw(const ascii_string_mref& mref,
                      std::size_t              pos,
                      const char*              str,
      ...
      void
      copy_string_raw(const unicode_string_mref& mref,
                      std::size_t                pos,
                      const char*                str,
                      std::size_t                len) const
      {
      ...
      void
      copy_string_raw(const byte_vector_mref& mref,
                      std::size_t             pos,
...
#endif /* end of include guard: CODEC_HELPER_H_QI11HSL8 */
