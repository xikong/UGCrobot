#ifndef FIELD_MASKS_H_ES8O7JY
#define FIELD_MASKS_H_ES8O7JY
...
namespace mfast
{
  namespace SQLite3
  {
    ...
    struct field_masks
    {
      field_masks(unsigned skip=0,
                  unsigned primary_key=0,
                  unsigned ordered=0)
        : skip_mask(skip)
        , primary_key_mask(primary_key)
      ...
      {
        return (inst->field_type() == field_type_exponent && inst->field_operator() == operator_constant);
      }
...
#endif /* end of include guard: FIELD_MASKS_H_ES8O7JY */
