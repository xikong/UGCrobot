#ifndef CONVERTER_CORE_H_KIXIUOS7
#define CONVERTER_CORE_H_KIXIUOS7

#include "../../../fast/mfast/sqlite3/field_masks.h"
...
namespace mfast
{
  namespace SQLite3
  {
    ...
    {
    public:
      converter_core(const char* db_file,
                const field_masks&);
      ...
      template <typename T>
      void bind(const template_info& info, unsigned column,  const int_cref<T>& ref) const
      {
        bind(info, column, ref.value());
      }

      void bind(const template_info& info, unsigned column,  const enum_cref& ref) const
      {
        bind(info, column, ref.value());
      }

      void bind(const template_info& info, unsigned column,  const decimal_cref& ref) const;
      void bind(const template_info& info, unsigned column,  const char* str) const;

      template <typename T>
      void bind(const template_info& info, unsigned column,  const string_cref<T>& ref) const
      {
        bind(info, column, ref.c_str());
      }

      void bind(const template_info& info, unsigned column,  const byte_vector_cref& ref) const;

      void bind_null(const template_info& info, unsigned column) const;

      void bind(const template_info& info, unsigned column, const field_cref& ref) const;
    private:
      sqlite3* db_;
...
#endif /* end of include guard: CONVERTER_CORE_H_KIXIUOS7 */
